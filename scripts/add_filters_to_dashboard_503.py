...
def main():
    ...
    print(f"Total cards processed: {len(cards_updated) + len(cards_skipped) + len(cards_failed)}")
    ...
    # Save results
    with open('dashboard_503_filter_update.json', 'w') as f:
        json.dump({
    ...
    print(f"\n💾 Results saved to dashboard_503_filter_update.json")
    print(f"\n🎯 All questions on dashboard {dashboard_id} now have Card_Geo and CARD_ISOCOUNTRY filters!")
