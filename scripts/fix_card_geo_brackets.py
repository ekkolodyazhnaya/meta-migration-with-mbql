#!/usr/bin/env python3
"""
Script to fix all cards that have '{Card_Geo}' instead of '{{Card_Geo}}' with double brackets
"""
...
def needs_bracket_fix(card_data):
    ...
    # Check if SQL has single bracket {Card_Geo} instead of double {{Card_Geo}}
    has_single_bracket = '{Card_Geo}' in query_text and '{{Card_Geo}}' not in query_text
    
    return has_single_bracket
...
def fix_card_brackets(session_token, card_id, card_data):
    ...
    try:
        ...
        updated_query = query_text.replace('{Card_Geo}', '{{Card_Geo}}')
        ...
        if update_response.status_code == 200:
            return True, "Fixed {Card_Geo} to {{Card_Geo}}"
        else:
...
def main():
    ...
    print(f"🔧 Fixing Card_Geo brackets on Dashboard {dashboard_id}...")
    print("=" * 60)
    ...
    for i, dashcard in enumerate(dashcards):
        ...
        if needs_fix:
            print(f"    ⚠️  Needs bracket fix: has {{Card_Geo}} instead of {{Card_Geo}}")
    ...
    # Save results
    with open('dashboard_485_bracket_fix.json', 'w') as f:
        json.dump({
    ...
    print(f"\n💾 Results saved to dashboard_485_bracket_fix.json")
